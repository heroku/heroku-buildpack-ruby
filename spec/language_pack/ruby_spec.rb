  ...
  describe '#compile' do
    ...
    context 'installing ruby' do
      ...
      before :each do
        ...
        subject.stub(:ruby_versions) { %w{
            ruby-2.0.0
            ruby-1.9.3
            ruby-1.9.2
            ruby-1.8.7
            ruby-1.9.3-jruby-1.7.0
            ruby-1.8.7-jruby-1.7.0
            ruby-1.9.3-jruby-1.7.1
            ruby-1.8.7-jruby-1.7.1
            ruby-1.9.3-rbx-2.0.0dev
      ...
      context 'when building ruby' do
        ...
        it 'downloads and untars the ruby and ruby-build tarballs' do
          subject.stub(:run).with("ln -s ../vendor/ruby-1.9.2/bin/a bin")
          subject.stub(:run).with("ln -s ../vendor/ruby-1.9.2/bin/b bin")

          subject.should_receive(:run).with('curl https://s3.amazonaws.com/heroku-buildpack-ruby/ruby-1.9.2.tgz -s -o - | tar zxf -') { %x{true} }
          subject.should_receive(:run).with('curl https://s3.amazonaws.com/heroku-buildpack-ruby/ruby-build-1.9.2.tgz -s -o - | tar zxf -') { %x{true} }
      ...
      context 'when not building ruby' do
        ...
        it 'symlinks the bin files' do
          subject.stub(:run).with('curl https://s3.amazonaws.com/heroku-buildpack-ruby/ruby-1.9.2.tgz -s -o - | tar zxf -') { %x{true} }
      ...
      context 'creating bin dir' do
        ...
        it 'creates a bin dir' do
          subject.stub(:run).with('curl https://s3.amazonaws.com/heroku-buildpack-ruby/ruby-1.9.2.tgz -s -o - | tar zxf -') { %x{true} }
          subject.stub(:run).with('curl https://s3.amazonaws.com/heroku-buildpack-ruby/ruby-build-1.9.2.tgz -s -o - | tar zxf -') { %x{true} }
          subject.stub(:run).with("ln -s ../vendor/ruby-1.9.2/bin/a bin")
          subject.stub(:run).with("ln -s ../vendor/ruby-1.9.2/bin/b bin")
      ...
      context 'installing the jvm' do
        ...
        context 'creating bin dir' do
          ...
          it 'creates a bin dir' do
            ...
            subject.stub(:run).with('curl https://s3.amazonaws.com/heroku-buildpack-ruby/ruby-build-1.9.2.tgz -s -o - | tar zxf -')
      ...
      context 'setting up profiled' do
        ...
        context 'when ruby is MRI' do
          ...
          it 'writes the path to profiled' do
            expect(@ruby_profile).to include "PATH=\"$HOME/bin:$HOME/#{slug_vendor_base}/bin:$PATH\""
          end
